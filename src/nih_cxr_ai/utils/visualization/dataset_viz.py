...
class DatasetVisualizer:
    ...
    def __init__(
        ...
        self.data_dir = data_dir if data_dir is not None else Path("../data/nih_chest_xray")
    ...
    def plot_train_val_distribution(self, 
        ...
        train_bars = plt.bar(x - width/2, 
                            list(train_freqs.values()), 
                            width, 
                            label='Train', 
                            color='#2ecc71')
        val_bars = plt.bar(x + width/2, 
    ...
    def plot_label_correlations(self, figsize: Tuple[int, int] = (10, 8)) -> None:
        ...
        for idx, name in self.LABEL_MAPPING.items():
            binary_df[name] = self.df["label_list"].apply(lambda x: 1 if idx in x else 0)
