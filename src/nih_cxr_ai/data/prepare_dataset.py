...
def prepare_nih_dataset(output_dir: str, full_dataset: bool = True, subset_size: Optional[int] = None) -> Dict:
    """
    ...
    try:
        ...
        stats['dataset_info']['class_distribution'] = pd.Series(all_labels).value_counts().to_dict()
        stats['dataset_info']['multi_label_stats'] = {
            'avg_labels_per_image': np.mean([len(labels) for labels in dataset['labels']]),
            'max_labels_per_image': max(len(labels) for labels in dataset['labels']),
            'common_combinations': pd.Series(label_combinations).value_counts().head(10).to_dict()
        }
        ...
        process_size = len(dataset) if full_dataset else min(subset_size or 1000, len(dataset))
        ...
        logger.info(f"Total processed: {stats['dataset_info']['processed_images']} images")
